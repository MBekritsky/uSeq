...
if(is.null(opt$min.confidence)) opt$min.confidence <- 0.95
if(is.null(opt$min.allele.fit)) opt$min.allele.fit <- 1e-7
if(is.null(opt$min.noise.fit))  opt$min.noise.fit  <- 1e-3
if(is.null(opt$max.p.null))     opt$max.p.null     <- 0.05
...
ultra.rare.file <- sprintf("%sultraRares.txt",opt$directory)
cat("SSC.family.ID","chr","pos","motif","ref.length","allele.length","allele.bias",
    ...
    "pro.allele.count","sib.allele.count","\n", sep="\t", file=ultra.rare.file)
counter <- list(ultra.rare.loci = 0, too.many.nulls = 0, passed.ultra.rare.alleles = 0)
...
for(i in 0:opt$intervals){
  ...
  for(l in seq.int(1,dim(em.geno$genotypes)[1])){
    ...
    if(any(allele.counts$Freq == 1)){
      counter$ultra.rare.loci <- counter$ultra.rare.loci + 1
      for(j in which(allele.counts$Freq == 1)){
        ultra.rare <- allele.counts[j,1]
        
        # find the parent with the ultra-rare allele
        parent <- which(genotype.slice[,1:2] == allele.counts[j,1] & is.parent & non.null, arr.ind = T)[1]
        parent.type <- as.character(person.info$relation[parent])
  
        family <- by.fams[which(by.fams == parent, arr.ind = T)[1], ]
        family.id        <- as.character(person.info[family[1],'family.id'])
        if(!(family.id %in% gatk.bad.families)){
          
          family.with.ultra.rare <- genotype.slice[family,]
          
          if(all(family.with.ultra.rare[,"confidence"] > opt$min.confidence) & 
             all(family.with.ultra.rare[,"allele.fit"] > opt$min.allele.fit) &
             all(family.with.ultra.rare[,"noise.fit"]  > opt$min.noise.fit) & 
             all(family.with.ultra.rare[,"p.null"] < opt$max.p.null)){
            
            
            counter$passed.ultra.rare.alleles <- counter$passed.ultra.rare.alleles + 1
            ...
            ultra.rare.locus.info  <- chunk.locus.info[chunk.locus.info$allele.no == ultra.rare & chunk.locus.info$locus.row.ind == l,
                                                       c("chr","pos","unit","ref.length","allele.no","bias")]
            ultra.rare.hwe.p.val <- hwe.pvals[chunk.start.locus + l - 1]
            inheritance.pattern <- apply(genotype.slice[family,1:2], 1, function(x) sum(x == ultra.rare))
            ...
            cat(as.character(person.info$family.id[parent]), unlist(ultra.rare.locus.info), ultra.rare.hwe.p.val, 
                context,context.info,good.par.count,good.par.alleles,family.type, inheritance.pattern,"\n",sep="\t",
                file=ultra.rare.file,append=T)
          }
        }
      }
    }
